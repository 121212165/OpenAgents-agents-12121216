...
async def verify_data_sources():
    ...
    try:
        ...
        print(f"健康检查: {health.get('healthy_count', 0)}/{health.get('total_count', 0)} 数据源健康")
...
async def verify_error_handling():
    ...
    try:
        ...
        if result.success:
            print(f"错误处理成功，使用数据源: {result.source}")
            return True
        else:
            print(f"查询失败: {result.error}")
            return False
